...
class SiMaskFormer(nn.Module):
    ...
    def from_config(cls, cfg):
        ...
        # Loss parameters:
        deep_supervision = cfg.MODEL.SiMaskFormer.DEEP_SUPERVISION                                 #True
        no_object_weight = cfg.MODEL.SiMaskFormer.NO_OBJECT_WEIGHT                                 #0.1

        # loss weights
        class_weight = cfg.MODEL.SiMaskFormer.CLASS_WEIGHT                                         #4.0
        cost_class_weight = cfg.MODEL.SiMaskFormer.COST_CLASS_WEIGHT                               #4.0

        dice_weight = cfg.MODEL.SiMaskFormer.DICE_WEIGHT                                           #5.0
        cost_dice_weight = cfg.MODEL.SiMaskFormer.COST_DICE_WEIGHT                                 #5.0

        mask_weight = cfg.MODEL.SiMaskFormer.MASK_WEIGHT                                           #5.0
        cost_mask_weight = cfg.MODEL.SiMaskFormer.COST_MASK_WEIGHT                                 #5.0
        
        box_weight = cfg.MODEL.SiMaskFormer.BOX_WEIGHT                                             #5.0
        cost_box_weight = cfg.MODEL.SiMaskFormer.COST_BOX_WEIGHT                                   #5.0
        
        giou_weight = cfg.MODEL.SiMaskFormer.GIOU_WEIGHT                                           #2.0
        cost_giou_weight = cfg.MODEL.SiMaskFormer.COST_GIOU_WEIGHT                                 #2.0

        proposal_weight = cfg.MODEL.SiMaskFormer.PROPOSAL_WEIGHT                                   #0.5
        ...
        matcher = HungarianMatcher(
            ...
            num_points=cfg.MODEL.SiMaskFormer.TRAIN_NUM_POINTS,                                    #112 * 112
        )
        ...
        if deep_supervision:
            dec_layers = cfg.MODEL.SiMaskFormer.DEC_LAYERS                                           #9
            aux_weight_dict = {}
            for i in range(dec_layers):
                aux_weight_dict.update({k + f"_{i}": v for k, v in weight_dict.items()})
            weight_dict.update(aux_weight_dict)
        if cfg.MODEL.SiMaskFormer.BOX_LOSS:                                                          #True
        ...
        #proposal predict
        weight_dict["proposal_loss"] = proposal_weight
        ...
        criterion = SetCriterion(
            ...
            losses=losses,
            num_points=cfg.MODEL.SiMaskFormer.TRAIN_NUM_POINTS,                                      #112 * 112
            oversample_ratio=cfg.MODEL.SiMaskFormer.OVERSAMPLE_RATIO,                                #3.0
            importance_sample_ratio=cfg.MODEL.SiMaskFormer.IMPORTANCE_SAMPLE_RATIO,                  #0.75
            dn=cfg.MODEL.SiMaskFormer.DN,                                                            #"seg"
            dn_losses=dn_losses,
            panoptic_on=cfg.MODEL.SiMaskFormer.PANO_BOX_LOSS,                                        #False
            semantic_ce_loss=cfg.MODEL.SiMaskFormer.TEST.SEMANTIC_ON and cfg.MODEL.SiMaskFormer.SEMANTIC_CE_LOSS and not cfg.MODEL.SiMaskFormer.TEST.PANOPTIC_ON,
            #                                           False                                 False                                               False                                               
        )
        ...
        return {
            ...
            "criterion": criterion,
            "num_queries": cfg.MODEL.SiMaskFormer.NUM_OBJECT_QUERIES,                                  #100
            "object_mask_threshold": cfg.MODEL.SiMaskFormer.TEST.OBJECT_MASK_THRESHOLD,                #0.25
            "overlap_threshold": cfg.MODEL.SiMaskFormer.TEST.OVERLAP_THRESHOLD,                        #0.8
            "metadata": MetadataCatalog.get(cfg.DATASETS.TRAIN[0]),                                   #*****
            "size_divisibility": cfg.MODEL.SiMaskFormer.SIZE_DIVISIBILITY,                             #32
            "sem_seg_postprocess_before_inference": (
                cfg.MODEL.SiMaskFormer.TEST.SEM_SEG_POSTPROCESSING_BEFORE_INFERENCE                    #False
                or cfg.MODEL.SiMaskFormer.TEST.PANOPTIC_ON                                             #False
                or cfg.MODEL.SiMaskFormer.TEST.INSTANCE_ON                                             #True
            ),
            "pixel_mean": cfg.MODEL.PIXEL_MEAN,                                                       #[103.530, 116.280, 123.675]
            "pixel_std": cfg.MODEL.PIXEL_STD,                                                         #[1.0, 1.0, 1.0]
            # inference
            "semantic_on": cfg.MODEL.SiMaskFormer.TEST.SEMANTIC_ON,                                    #False
            "instance_on": cfg.MODEL.SiMaskFormer.TEST.INSTANCE_ON,                                    #True
            "panoptic_on": cfg.MODEL.SiMaskFormer.TEST.PANOPTIC_ON,                                    #False
            "test_topk_per_image": cfg.TEST.DETECTIONS_PER_IMAGE,                                     #100
            "data_loader": cfg.INPUT.DATASET_MAPPER_NAME,                                             #"MaskDINO_semantic"
            "focus_on_box": cfg.MODEL.SiMaskFormer.TEST.TEST_FOUCUS_ON_BOX,                            #False
            "transform_eval": cfg.MODEL.SiMaskFormer.TEST.PANO_TRANSFORM_EVAL,                         #True
            "pano_temp": cfg.MODEL.SiMaskFormer.TEST.PANO_TEMPERATURE,                                 #0.06
            "semantic_ce_loss": cfg.MODEL.SiMaskFormer.TEST.SEMANTIC_ON and cfg.MODEL.SiMaskFormer.SEMANTIC_CE_LOSS and not cfg.MODEL.SiMaskFormer.TEST.PANOPTIC_ON,
            #                                           False                                 False                                               False 
            "ignore_keep": cfg.MODEL.SiMaskFormer.SEMANTIC_CE_LOSS_KEEP_IGNORE
        }
