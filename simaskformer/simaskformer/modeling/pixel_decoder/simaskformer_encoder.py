...
def build_pixel_decoder(cfg, input_shape):
    """
    Build a pixel decoder from `cfg.MODEL.SEM_SEG_HEAD.PIXEL_DECODER_NAME`.
    """
    name = cfg.MODEL.SEM_SEG_HEAD.PIXEL_DECODER_NAME                                      #SiMaskFormerEncoder
...
class SiMaskFormerEncoder(nn.Module):
    """
    ...
    def __init__(
        ...
        *,
        ...
        """
        super().__init__()
        transformer_input_shape = {                                                                                           #Shape:'res3', 'res4', 'res5'
        ...
        self.feature_channels = [v.channels for k, v in input_shape]                                                          #[256, 512, 1024, 2048]
        self.feature_order = feature_order

        if feature_order == "low2high":
            transformer_input_shape = sorted(transformer_input_shape.items(), key=lambda x: -x[1].stride)
        else:
            transformer_input_shape = sorted(transformer_input_shape.items(), key=lambda x: x[1].stride)                      #Shape:'res3', 'res4', 'res5'
        ...
        self.transformer_feature_strides = [v.stride for k, v in transformer_input_shape]  # to decide extra FPN layers       #[8, 16, 32]

        self.num_feature_levels = num_feature_levels  # always use 3 scales                                                   #3
        self.total_num_feature_levels = total_num_feature_levels                                                              #3
        self.common_stride = common_stride                                                                                    #4

        self.transformer_num_feature_levels = len(self.transformer_in_features)                                               #3
        self.low_resolution_index = transformer_in_channels.index(max(transformer_in_channels))                               #2
        self.high_resolution_index = 0 if self.feature_order == 'low2high' else -1                                            #-1 ???
    ...
    def from_config(cls, cfg, input_shape: Dict[str, ShapeSpec]):
        ...
        ret["transformer_dropout"] = cfg.MODEL.SiMaskFormer.DROPOUT                                                                       #0.0
        ret["transformer_nheads"] = cfg.MODEL.SiMaskFormer.NHEADS                                                                         #8
        ret["transformer_dim_feedforward"] = cfg.MODEL.SEM_SEG_HEAD.DIM_FEEDFORWARD  # deformable transformer encoder                   #1024
        ret[
            "transformer_enc_layers"
        ] = cfg.MODEL.SEM_SEG_HEAD.TRANSFORMER_ENC_LAYERS  # a separate config                                                          #6
        ...
        ret["num_feature_levels"] = cfg.MODEL.SEM_SEG_HEAD.NUM_FEATURE_LEVELS                                                           #3
        ret["feature_order"] = cfg.MODEL.SEM_SEG_HEAD.FEATURE_ORDER                                                                     #'high2low'
        return ret
    ...
    def forward_features(self, features, masks):
        ...
        for idx, f in enumerate(self.in_features[:self.num_fpn_levels][::-1]):
            ...
            y =torch.cat((cur_fpn , F.interpolate(out[self.high_resolution_index], size=cur_fpn.shape[-2:], mode="bilinear", align_corners=False)), dim=1) 
