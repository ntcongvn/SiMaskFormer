...
from simaskformer.utils import box_ops
...
class SetCriterion(nn.Module):
    ...
    def forward(self, outputs, targets, mask_dict=None):
        ...
        #Total Cost
        bs_total_C=None
        bs_total_C = self.matcher.Compute_Cost_Only(outputs_without_aux, targets)
        if "aux_outputs" in outputs:
            for i, aux_outputs in enumerate(outputs["aux_outputs"]):
                bs_aux_total_C = self.matcher.Compute_Cost_Only(aux_outputs, targets)
                for i in range(len(bs_aux_total_C)):
                    bs_total_C[i] = bs_total_C[i] + bs_aux_total_C[i]
        indices = self.matcher.Compute_Indices_Only(bs_total_C)        
        ...
        if 'interm_outputs' in outputs:
            ...
            indices, indices_Cost = self.matcher(interm_outputs, targets,return_min_cost_per_query=True)
            for loss in self.losses:
                l_dict = self.get_loss(loss, interm_outputs, targets, indices, num_masks)
                l_dict = {k + f'_interm': v for k, v in l_dict.items()}
                losses.update(l_dict)

            propasal_cost=torch.stack(indices_Cost)

            gt_costs=propasal_cost

            min_cost = gt_costs.min(dim=1, keepdim=True)[0]  # shape (B, 1)
            max_cost = gt_costs.max(dim=1, keepdim=True)[0]  # shape (B, 1)
            gt_costs_norm = (gt_costs - min_cost) / (max_cost - min_cost + 1e-6)

            pred_costs=outputs["pred_proposal_cost"]

            proposal_loss=F.l1_loss(pred_costs, gt_costs_norm)
            losses["proposal_loss"]=proposal_loss
