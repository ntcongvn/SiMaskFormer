...
# ------------------------------------------------------------------------
"""
SiMakFormer Training Script based on and MaskDINO, Mask2Former.
"""
import warnings
...
from simaskformer.modeling.lr_schedule import build_custom_lr_scheduler

from detectron2.engine.hooks import EvalHook


#Custom hooks with for evaluating afer certain iters - cfg.TEST.EVAL_START_ITER
class EvalHook_Custom(EvalHook):
    def __init__(self, eval_period, eval_function, eval_after_train=True,eval_start_iter=None):
        super().__init__(eval_period, eval_function, eval_after_train=True)
        self._eval_start_iter=eval_start_iter
    
    def after_step(self):
        next_iter = self.trainer.iter + 1
        if self._eval_start_iter is not None and self._eval_start_iter > next_iter:
          None
        else:
...
class Trainer(DefaultTrainer):
    ...
    def build_evaluator(cls, cfg, dataset_name, output_folder=None):
        ...
        if evaluator_type== "polyp_sem_seg":
          evaluator_list.append(PolypDBSemSegEvaluator(
                    ...
                    visualize=cfg.MODEL.SiMaskFormer.TEST.VISUALIZE,
                    visualize_path=output_folder+"/visualize",
                )
          )
        # semantic segmentation  - VoFo Semantic Segmentation
        if evaluator_type== "vofo_sem_seg":
          evaluator_list.append(VoFoSemSegEvaluator(
                    dataset_name,
                    distributed=True,
                    output_dir=output_folder,
                    visualize=cfg.MODEL.SiMaskFormer.TEST.VISUALIZE,
        ...
        if evaluator_type== "isic_sem_seg":
          evaluator_list.append(ISICSemSegEvaluator(
                    dataset_name,
                    distributed=True,
                    output_dir=output_folder,
                    visualize=cfg.MODEL.SiMaskFormer.TEST.VISUALIZE,
    ...
    def build_train_loader(cls, cfg):
        ...
        # Polyp semantic segmentation dataset mapper
            ...
            return build_detection_train_loader(cfg, mapper=mapper)
        elif cfg.INPUT.DATASET_MAPPER_NAME == "vofo_semantic":
            mapper = VoFoSemanticDatasetMapper(cfg, True)
            return build_detection_train_loader(cfg, mapper=mapper)
        elif cfg.INPUT.DATASET_MAPPER_NAME == "isic_semantic":
            mapper = ISICSemanticDatasetMapper(cfg, True)
            return build_detection_train_loader(cfg, mapper=mapper)
        else:
            mapper = None
            return build_detection_train_loader(cfg, mapper=mapper)
    ...
    def build_lr_scheduler(cls, cfg, optimizer):
        ...
        """
        if cfg.SOLVER.LR_SCHEDULER_NAME in ["CosineAnnealingWarmRestarts","CosineAnnealingLR"]:
            return build_custom_lr_scheduler(cfg, optimizer)
        else:
    ...
    def build_hooks(self):
        ...
        ret.append(EvalHook_Custom(cfg.TEST.EVAL_PERIOD, test_and_save_results,eval_start_iter=cfg.TEST.EVAL_START_ITER))
...
def setup(args):
    ...
    add_simaskformer_config(cfg)
    cfg.merge_from_file(args.config_file)
    cfg.merge_from_list(args.opts)
    cfg.freeze()
    default_setup(cfg, args)
    setup_logger(output=cfg.OUTPUT_DIR, distributed_rank=comm.get_rank(), name="simaskformer")
    return cfg
